...
const TransactionPage = () => {
  const columns = [
    {
      ...
      key: 'date',
      render: (text) => (
        <Space>{moment(text).tz('Asia/Ho_Chi_Minh').format('HH:mm:ss DD-MM-YYYY')}</Space>
      )
    },
    {
      title: 'Số tiền',
    ...
    },
    {
      ...
      dataIndex: 'status',
      key: 'status',
      render: (text) => <Space>{text === 1 ? 'Tiền ra' : 'Tiền vào'}</Space>
    }
  ]
  ...
  const sortedTransactions =
    user && user.user && user.user.transactionHistory
